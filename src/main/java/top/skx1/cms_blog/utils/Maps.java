...
/*
 * @Author shaokexiang
 * @Date 2021/1/31 12:56
 * @Description //Map工具类，让Map支持链式调用
 * @Param
 * @return
 **/
...
public class Maps {
    ...
    //1、build方法目的是构建Maps对象，因为Maps对象有一个Map集合（支持构建函数）
    ...
    //2、build方法目的是构建Maps对象，因为Maps对象有一个Map集合（支持构建简单的Map对象）
    ...
    public <T> Map<String,Object> beanToMapForUpdate(T bean){
        if(bean != null){
            ...
            for (Object key:beanMap.keySet()){
                paramMap.put("update"+uppperFirstletter(key+""),beanMap.get(key));
            }
        }
        return paramMap;
    }
    ...
    public static String uppperFirstletter(String str){
